...
export default function TaskItems({ task }: TodoItemProps) {
  ...
  return (
    <Card
      className={cn(
        "p-4 flex items-center gap-3 border-l-4 transition-all duration-300",
        task.completed
    ...
    >
      ...
      {isEditing ? (
        <div className="flex-1 flex gap-2">
          ...
          <Button size="icon" variant="ghost" onClick={handleSave} className="text-green-600">
            <Save className="h-4 w-4" />
          </Button>
          <Button size="icon" variant="ghost" onClick={handleCancel} className="text-red-600">
            <X className="h-4 w-4" />
          </Button>
        </div>
      ) : (
